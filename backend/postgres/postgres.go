...
func (b *postgresBackend) RemoveWorkflowInstances(ctx context.Context, options ...backend.RemovalOption) error {
	...
	rows, err := b.db.QueryContext(ctx, SQLReplacer(`SELECT instance_id, execution_id FROM instances WHERE completed_at < ?`), ro.FinishedBefore)
	if err != nil {
		return err
	}
...
func createInstance(ctx context.Context, tx *sql.Tx, queue workflow.Queue, wfi *workflow.Instance, metadata *workflow.Metadata) error {
	...
	if err := tx.QueryRowContext(
		ctx,
		SQLReplacer("SELECT 1 FROM instances WHERE instance_id = ? AND state = ? LIMIT 1"),
		wfi.InstanceID,
	...
	_, err = tx.ExecContext(
		ctx,
		SQLReplacer("INSERT INTO instances (queue, instance_id, execution_id, parent_instance_id, parent_execution_id, parent_schedule_event_id, metadata, state) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"),
		string(queue),
		wfi.InstanceID,
		wfi.ExecutionID,
		parentInstanceID,
...
func (b *postgresBackend) GetWorkflowTask(ctx context.Context, queues []workflow.Queue) (*backend.WorkflowTask, error) {
	...
	query := SQLReplacer(fmt.Sprintf(`SELECT i.id, i.queue, i.instance_id, i.execution_id, i.parent_instance_id, i.parent_execution_id, i.parent_schedule_event_id, i.metadata, i.sticky_until
	FROM instances i
...
func (b *postgresBackend) GetActivityTask(ctx context.Context, queues []workflow.Queue) (*backend.ActivityTask, error) {
	...
	res := tx.QueryRowContext(
		ctx,
		SQLReplacer(fmt.Sprintf(`SELECT a.id, a.activity_id, a.instance_id, a.execution_id, a.queue,
...
func (b *postgresBackend) CompleteActivityTask(ctx context.Context, task *backend.ActivityTask, result *history.Event) error {
	...
	if res, err := tx.ExecContext(
		ctx,
		SQLReplacer(`DELETE FROM activities WHERE activity_id = ? AND instance_id = ? AND execution_id = ? AND worker = ? AND queue = ?`),
		task.ActivityID,
		task.WorkflowInstance.InstanceID,
		task.WorkflowInstance.ExecutionID,
		b.workerName,
